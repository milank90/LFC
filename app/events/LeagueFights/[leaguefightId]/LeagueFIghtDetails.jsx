...
const LeagueFightDetails = ({ leaguefightId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (leaguefightId) {
          const leaguefightDocRef = doc(db, "LeagueFights", leaguefightId);
          const leaguefightDocSnap = await getDoc(leaguefightDocRef);

          if (leaguefightDocSnap.exists()) {
            const leaguefightDetails = { id: leaguefightDocSnap.id, ...leaguefightDocSnap.data() };
            setDetails(leaguefightDetails);
          } else {
        ...
        <div className="d-flex flex-column flex-md-row">
          <div className="col-md-3 col-12 fighter-profile order-2 order-lg-1">
            <div className="fighter-info">
              <p>
                <strong>{details.leaguefight_name}</strong>
              </p>
              <p style={{ fontSize: '30px', textTransform: 'uppercase', overflowWrap: 'break-word' }}>
                <strong>{details.leaguefight_date}</strong>
              </p>
              ...
              <p>
                {details.eleaguefight_datacenter}/{details.leaguefight_world}
              </p>
            </div>
...
export default LeagueFightDetails;
